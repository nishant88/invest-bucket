...
export const VentureHealth: React.FC = () => {
  ...
  return (
    <div className="space-y-5.5 text-left pb-16 animate-fade-in text-[#0d1c32] select-none w-full px-1">
      
      {/* SECTION 1: Core Health Indicators Stack */}
      <section className="space-y-4.5">
        {/* Card 1: Venture Health Score */}
        <div className="bg-white rounded-[28px] p-6.5 card-shadow border border-outline-variant/40 relative overflow-hidden w-full h-[175px] flex flex-col justify-between">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-baseline gap-1 mt-4.5">
                <span className="text-[46px] font-black text-[#0d1c32] tracking-tight leading-none">{healthData.score}</span>
        ...
        <div className="bg-[#0d1c32] text-white rounded-[28px] p-6.5 card-shadow border border-white/5 flex items-center justify-between relative overflow-hidden w-full h-[175px]">
          ...
          {/* SVG Progress Ring */}
          <div className="relative w-18 h-18 shrink-0 flex items-center justify-center bg-white/5 rounded-full">
            <svg className="w-full h-full transform -rotate-90">
              ...
              <circle 
                ...
                strokeDashoffset={2 * Math.PI * 30 * (1 - healthData.successProbability / 100)}
                strokeLinecap="round"
              />
            </svg>
            <div className="absolute text-center select-none">
              <span className="text-[14.5px] font-black text-white leading-none">{healthData.successProbability}%</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4.5">
        ...
        <div className="bg-white rounded-[28px] p-6.5 card-shadow border border-outline-variant/40 flex flex-col justify-between w-full h-[205px]">
          ...
          <div className="relative flex flex-col items-center justify-center flex-1">
            <div className="relative w-48 h-24 overflow-hidden flex items-end">
              <svg className="w-48 h-48 absolute bottom-0 left-0">
                <circle cx="96" cy="96" r="84" stroke="#f1f5f9" strokeWidth="12.5" strokeDasharray="264 264" strokeDashoffset="0" fill="transparent" strokeLinecap="round" />
                <circle cx="96" cy="96" r="84" stroke={healthData.status === 'Green' ? '#10b981' : healthData.status === 'Amber' ? '#f59e0b' : '#f43f5e'} strokeWidth="12.5" strokeDasharray="264 264" strokeDashoffset={264 - (264 * (healthData.score / 100))} fill="transparent" strokeLinecap="round" />
              </svg>
              <div 
                className="absolute bottom-0 left-1/2 w-1.5 h-20 bg-[#0d1c32] origin-bottom -translate-x-1/2 transition-all duration-1000 rounded-t-full"
                style={{ transform: `translateX(-50%) rotate(${gaugeAngle}deg)` }}
              />
              <div className="absolute bottom-0 left-1/2 w-4.5 h-4.5 bg-[#0d1c32] rounded-full border-2 border-white -translate-x-1/2 translate-y-2 z-10" />
            </div>
          </div>
        ...
        <div className="bg-white rounded-[28px] p-6.5 card-shadow border border-outline-variant/40 w-full h-[225px] flex flex-col justify-between">
          <h4 className="font-display font-bold text-[11px] text-slate-500 uppercase tracking-wider block">Risk Severity Table</h4>
          
          <div className="grid grid-cols-2 gap-3.5 flex-1 mt-3.5">
            <div className="p-4 rounded-2xl border border-outline-variant/20 flex flex-col justify-between bg-slate-50/60 h-[82px]">
              <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider leading-none">Financial Splits</span>
              <span className="text-[11.5px] font-extrabold text-[#0d1c32] uppercase mt-2">Low Variance</span>
            </div>

            <div className="p-4 rounded-2xl border border-outline-variant/20 flex flex-col justify-between bg-slate-50/60 h-[82px]">
              <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider leading-none">Dispute Count</span>
              <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-md inline-block w-fit uppercase mt-2 ${
            ...
            <div className="p-4 rounded-2xl border border-outline-variant/20 flex flex-col justify-between bg-slate-50/60 h-[82px]">
              <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider leading-none">Budget Overruns</span>
              <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-md inline-block w-fit uppercase mt-2 ${
            ...
            <div className="p-4 rounded-2xl border border-outline-variant/20 flex flex-col justify-between bg-slate-50/60 h-[82px]">
              <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider leading-none">Phase Locks</span>
              <span className="text-[11.5px] font-black text-[#0d1c32] uppercase mt-2">
      ...
      <section className="space-y-4.5">
        ...
        <div className="bg-[#ecf3e3] rounded-[28px] p-6.5 border border-outline-variant/30 text-[#0d1c32] relative overflow-hidden flex flex-col justify-between w-full min-h-[175px]">
          <div className="absolute right-4 top-4 select-none pointer-events-none opacity-20">
            <span className="material-symbols-outlined text-[#0d1c32] text-[64px]">insights</span>
          </div>
          ...
          <div className="space-y-2 relative z-10">
            ...
            <p className="text-[12.5px] font-bold leading-relaxed text-[#0d1c32]/90 mt-2.5 pr-14 min-h-[56px] flex items-center">
              {healthData.insights[activeInsightIndex]}
            </p>
          </div>

          <div className="flex items-center justify-between border-t border-[#0d1c32]/10 pt-3.5 mt-4 relative z-10">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="w-7.5 h-7.5 bg-white rounded-full flex items-center justify-center border border-black/5 active:scale-90 transition-all shadow-sm"
              >
        ...
        <div className="bg-white rounded-[28px] p-6.5 card-shadow border border-outline-variant/40 flex flex-col justify-between w-full h-[200px]">
          <div className="flex justify-between items-center pb-3 border-b border-outline-variant/25">
            ...
            <button 
              ...
              className="flex items-center gap-1.5 bg-surface border border-outline-variant/20 rounded-xl px-3 py-1 text-[9px] font-black text-[#0d1c32] hover:bg-slate-50 active:scale-95 transition-all shadow-sm"
            >
          ...
          <blockquote className="text-[11.5px] font-semibold text-slate-600 leading-relaxed italic text-left mt-3.5 flex-1 overflow-y-auto max-h-[115px]">
      ...
      <section className="bg-white rounded-[28px] p-6.5 card-shadow border border-outline-variant/40 space-y-5 w-full">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4.5 bg-surface rounded-2xl border border-outline-variant/15 space-y-2.5 min-h-[115px] flex flex-col justify-between">
            ...
            <ul className="list-disc pl-5 text-[11px] font-medium text-slate-600 space-y-1.5 leading-normal flex-1">
      ...
      <section className="space-y-4.5 w-full">
        ...
        <div className="bg-white rounded-[28px] p-6.5 card-shadow border border-outline-variant/40 w-full h-[185px] flex flex-col justify-between">
